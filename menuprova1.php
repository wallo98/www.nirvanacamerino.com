...
<style>
    ...
    /* CSS per le card prodotti caricate via AJAX */
    .services-wrap .img {
        min-height: 250px;
        height: 250px;
    }
    ...
    /* Forza visibilità delle card caricate via AJAX */
    #menu-container .ftco-animate {
        opacity: 1 !important;
    ...
    .category-btn.active {
        background: linear-gradient(135deg, #fac564 0%, #f9b84d 100%) !important;
        color: #1a1a1a !important;
        border-color: #fac564 !important;
        box-shadow: 0 4px 15px rgba(250, 197, 100, 0.4), 
                    0 0 0 2px rgba(250, 197, 100, 0.2) inset !important;
        transform: translateY(-2px);
        font-weight: 600;
    }
    
    .category-btn.active:hover {
        background: linear-gradient(135deg, #f9b84d 0%, #f8ab3a 100%) !important;
        box-shadow: 0 6px 20px rgba(250, 197, 100, 0.5), 
                    0 0 0 2px rgba(250, 197, 100, 0.3) inset !important;
...
<script>
    ...
    document.querySelectorAll('.category-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            // Carica i prodotti della categoria selezionata via AJAX
            caricaProdottiDaAction(categoria);
        });
    });
    ...
    $(document).ready(function() {
        ...
        // Imposta "Pizze Rosse" come categoria di default e attiva il pulsante
        document.querySelector('.category-btn[data-category="Pizze Rosse"]').classList.add('active');
        caricaProdottiDaAction("Pizze Rosse");
    });
    ...
    // Recupera le card dei prodotti da action.php via AJAX e inseriscile nel menu
    function caricaProdottiDaAction(categoria) {
        $.ajax({
            type: "POST",   
            url: "action.php",
            data: {
                _action: "get_cards_html",
                categoria: categoria
            },
            dataType: "json",
            success: function (response) {
                console.log(response);
                // Inserisci l'html delle card prodotti nel contenitore (adatta l'id se serve)
                $("#menu-container").html(response.data.html);
            },
            error: function () {
                console.error("Errore nel recupero delle card dei prodotti.");
            }
        });
    }
